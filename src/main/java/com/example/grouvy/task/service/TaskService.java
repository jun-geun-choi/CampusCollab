    ...
    public void createTask(TaskForm taskForm) {
        ...
        if (localFiles != null && !localFiles.isEmpty()) {
            ...
            for (MultipartFile localFile : localFiles) {
                ...
                try {
                    ...
                    File dir = new File(saveDirectory + "/task");
                    if (!dir.exists()) dir.mkdirs(); // 폴더 없으면 생성

                    File dest = new File(dir, fileName);
                    localFile.transferTo(dest); // 로컬에 파일 저장
        ...
        if (existingFileIds != null && !existingFileIds.isEmpty()) {
            ...
            for (Integer existingFileId : existingFileIds) {
                ...
                try {
                    // 원본 파일
                    File sourceFile = new File(saveDirectory + "/" + originalFile.getOwnerType(), originalFile.getStoredName());

                    // 복사할 대상 폴더
                    File destDir = new File(saveDirectory + "/task");
                    if (!destDir.exists()) destDir.mkdirs();

                    File targetFile = new File(destDir, newStoredName);

                    // 파일 복사
                    Files.copy(sourceFile.toPath(), targetFile.toPath(), StandardCopyOption.REPLACE_EXISTING);

                } catch (IOException e) {
                    throw new RuntimeException("문서함 파일 복사 중 오류 발생", e);
                }
