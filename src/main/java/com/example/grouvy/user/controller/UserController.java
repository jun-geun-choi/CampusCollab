...
public class UserController {
    ...
    /**
     * 사용자가 마이페이지에서 "프로필 변경" 클릭 후 화면 바꿀 때
     * @param dto userId(사용자 식별자), image(변경할 파일)
     * @return
     * @throws IOException
     */
    ...
    public String updateProfile(@ModelAttribute ProfileRequest dto) throws IOException {
        ...
        // 변경된 내용을 바탕으로 SecurityUser 업데이트
        SecurityUser updatedSecurityUser = new SecurityUser(updatedUser);
        Authentication newAuth = new UsernamePasswordAuthenticationToken
               (updatedSecurityUser,
                updatedSecurityUser.getPassword(),
                updatedSecurityUser.getAuthorities());
        SecurityContextHolder.getContext().setAuthentication(newAuth);
