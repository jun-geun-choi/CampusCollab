...
//            uploadedFile.transferTo(dest);
            ...
            // 저장 폴더 지정
            File dir = new File(saveDirectory + "/" + file.getOwnerType());
            if (!dir.exists()) dir.mkdirs(); // 폴더 없으면 생성

            File dest = new File(dir, filename);
            uploadedFile.transferTo(dest); // 실제 파일 저장
        } catch (Exception e) {
...
/*            storage.delete(bucketName, storedName);*/
            File fileToDelete = new File(saveDirectory + "/" + ownerType, storedName);
    ...
    public File getDownloadFile(int fileId) {
        ...
        // 실제 저장된 로컬 경로
        File file = new File(saveDirectory + "/" + fileVo.getOwnerType(), storedName);

        // 존재 여부 검사
        if (!file.exists()) {
            throw new AppException("파일이 존재하지 않습니다: " + file.getAbsolutePath());
        }
